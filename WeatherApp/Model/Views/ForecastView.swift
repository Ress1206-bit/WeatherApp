                    ...
                    ScrollView(.horizontal, showsIndicators: false){
                        HStack {
                            ...
                            VStack{
                                ...
                                Text(" ")
                                        .font(.system(size: 15))
                                Text("\(weatherModel.convertToCelsius(temperature: Int(weatherData?.current.temp_f ?? 0)))°")
                                    .padding(.top, 0.25)
                                    .foregroundStyle(.white)
                                    .fontWeight(.semibold)
                            }
                                    ...
                                    ForEach(hourlyData, id: \.time_epoch) { hour in
                                        if Int(hour.time_epoch ?? 00) > Int(weatherData?.location.localtime_epoch ?? 01) {
                                            ...
                                            VStack{
                                                ...
                                                Text("\(weatherModel.convertToCelsius(temperature: Int(hour.temp_f ?? 0)))°")
                                                    .padding(.top, 0.25)
                                                    .foregroundStyle(.white)
                                                    .fontWeight(.semibold)
                                            }
                            ...
                            ForEach(Array(dailyData.enumerated()), id: \.element.date_epoch) { index, dayData in
                                HStack() {
                                    ...
                                    Image(systemName: weatherModel.conditionCodeIntoSFSymbol(code: Int(dayData.day?.condition?.code ?? 000), isDaytime: true))
                                        ...
                                        .scaledToFit()
                                    Spacer()
                                    Text("\(weatherModel.convertToCelsius(temperature: Int(dayData.day?.maxtemp_f ?? 00)))º↑ ")
                                        .frame(maxWidth: 50, alignment: .trailing)
                                        .fontWeight(.semibold)
                                    Text("\(weatherModel.convertToCelsius(temperature: Int(dayData.day?.mintemp_f ?? 00)))º↓ ")
                                        .frame(maxWidth: 50, alignment: .trailing)
